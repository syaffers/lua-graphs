...
// load json
d3.json("data/authors.json", function (err, data) {
  var i, dns = {};
  dns.name = "Uploads";
  dns.children = new Array();

  for(i = 0; i < data.length; i++) {
    var dnsItem = {};
    dnsItem.name = data[i][0];
    dnsItem.value = Number(data[i][1]);
    dns.children.push(dnsItem);
  }

  var node = svg.selectAll(".node")
    .data(bubble.nodes(dns).filter(function(d) { return !d.children; }))
    .enter()
    .append("g")
    .attr("class", "node")
