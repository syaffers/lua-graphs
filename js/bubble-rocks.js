...
// load json
d3.json("data/rocks.json", function (err, data) {
  var i, rocks = {};
  rocks.name = "LuaRocks";
  rocks.children = new Array();

  for(i = 0; i < data.length; i++) {
    var rocksItem = {};
    rocksItem.name = data[i][0];
    rocksItem.value = Number(data[i][1]);
    rocksItem.license = data[i][2];
    rocksItem.uploader = data[i][3];
    rocks.children.push(rocksItem);
  }

  var node = svg.selectAll(".node")
    .data(bubble.nodes(rocks).filter(function(d) { return !d.children; }))
    .enter()
    .append("g")
    .attr("class", "node")
  ...
  node.append("title")
    .text(function(d) {
      ...
      titleText += "Uploader: " + d.uploader + "\n";
      titleText += "Downloads: " + format(d.value) + "\n";
      titleText += "License: " + d.license;
      return titleText;
    });

  node.append("circle")
    .attr("r", function(d) { return d.r; })
    .style("fill", function(d) { return colour(d.uploader); });
