...
d3.json("data/authors.json", function (err, data) {
  var i, ups = {};
  ups.name = "Uploads";
  ups.children = new Array();

  for(i = 0; i < data.length; i++) {
    var upsItem = {};
    upsItem.name = data[i][0];
    upsItem.value = Number(data[i][2]);
    ups.children.push(upsItem);
  }

  var node = svg.selectAll(".node")
    .data(bubble.nodes(ups).filter(function(d) { return !d.children; }))
    .enter()
    .append("g")
    .attr("class", "node")
  ...
  node.append("title")
    .text(function(d) {
      ...
      titleText += "Uploads: " + format(d.value);
      return titleText;
    });
